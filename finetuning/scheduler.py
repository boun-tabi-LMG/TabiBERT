...
def _raise_if_schedule_and_max_incompatible(t_schedule: Time[int],
                                            t_max: Time[int],
                                            schedule_name: str):
...
def _linear_schedule(x: float,
                     start_y: float = 1.0,
                     finish_y: float = 0.0) -> float:
...
def _inverse_sqrt_schedule(x: Union[int, float],
                           alpha: float = 1.0,
...
class WarmupStableDecayScheduler(ComposerScheduler):
    ...
    def __init__(
        ...
        self.warmup_scheduler = LinearScheduler(alpha_i=1e-10,
                                                alpha_f=1.0,
                                                t_max=t_warmup)
...
class CosineInverseSqrtScheduler(ComposerScheduler):
    ...
    def __call__(self, state: State, ssr: float = 1.0):
        assert state.max_duration is not None, "max_duration should be set whenever schedulers are invoked"
        t_warmup = _convert_time(self.t_warmup,
                                 state,
                                 ssr=ssr if self.scale_schedules else 1)
        t_cooldown = _convert_time(self.t_cooldown,
                                   state,
                                   ssr=ssr if self.scale_schedules else 1)
        t_cosine = _convert_time(self.t_cosine,
                                 state,
                                 ssr=ssr if self.scale_schedules else 1)
        t_max = _convert_time(self.t_max, state, ssr=ssr)
        ...
        if self._last_t_max != t_max:
            ...
            _raise_if_schedule_and_max_incompatible(t_cooldown, t_max,
                                                    "cooldown")
        ...
        if v_step < v_warmup:
            return self._warmup_schedule(v_step / v_warmup,
                                         start_y=self.alpha_s,
                                         finish_y=1)
        ...
        if v_adjusted_step <= v_cosine:
            return _cosine_schedule(0.5 * v_adjusted_step / v_cosine,
                                    finish_y=self.alpha_f)
        else:
            return _inverse_sqrt_schedule(v_adjusted_step, self.gamma,
                                          self.delta)

    def _finish_setup(self, t_max: Time[int], t_warmup: Time[int],
                      t_cosine: Time[int], t_cooldown: Time[int]):
        self.delta = t_cosine.value * (1 + self.alpha_f - math.pi *
                                       (1 - self.alpha_f)) / (
                                           math.pi * (1 - self.alpha_f))
        self.gamma = (1 + self.alpha_f) / 2 * math.sqrt(t_cosine.value +
                                                        self.delta)

        self.cooldown_start = _inverse_sqrt_schedule(
            (t_max - t_cooldown - t_warmup).value, self.gamma, self.delta)
...
class OneMinusSqrtScheduler(ComposerScheduler):
    ...
    def __call__(self, state: State, ssr: float = 1.0):
        ...
        if current_time <= t_max - t_decay:
            ...
            lr_scale = self.alpha_f + (1 - self.alpha_f) * (
                1 - math.sqrt(relative_time.value))
