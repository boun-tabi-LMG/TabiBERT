...
# Duplicated from https://github.com/mosaicml/examples/blob/main/examples/benchmarks/bert/src/text_data.py .
...
def build_text_dataloader(
    ...
    mlm_probability = cfg.dataset.get('mlm_probability', 0.15)
...
if __name__ == '__main__':
    ...
    cfg = {
        ...
        'dataset': {
            ...
            'predownload': 1000,  # smaller predownload buffer
            'download_timeout': 120,  # longer timeout
            'num_canonical_nodes': 1,  # for single node setup
            'partition_algo': 'orig',
        },
        'drop_last': False,
        'num_workers': 4,
    ...
    # Add remote path if not specified
    if args.remote_path is None and not os.path.exists(os.path.join(args.local_path, args.split)):
        raise ValueError(
            f"Dataset not found at {os.path.join(args.local_path, args.split)}. "
            "Please either:\n"
            "1. Ensure the dataset exists locally with the correct MDS structure\n"
            "2. Provide a --remote_path argument to download from\n"
            "3. Convert your dataset to MDS format first"
        )

    if args.tokenizer_path is not None:
        print(f"Building tokenizer from local path: {args.tokenizer_path}")
        tokenizer = build_local_tokenizer(args.tokenizer_path)
    else:
        print(f"Building tokenizer from HuggingFace: {args.tokenizer}")
...
# Example usage:
# python dataset/test_data_loader.py --local_path dataset/my-copy-c4 --tokenizer answerdotai/ModernBERT-base --split train
# python dataset/test_data_loader.py --local_path dataset/my-copy-c4 --tokenizer boun-tabilab/TabiBERT --split train
# python dataset/test_data_loader.py --local_path dataset/sample/ --tokenizer answerdotai/ModernBERT-base --split test
# python dataset/test_data_loader.py --local_path dataset/sample/ --tokenizer boun-tabilab/TabiBERT --split test

# python dataset/test_data_loader.py --local_path dataset/sample/ --tokenizer boun-tabilab/TabiBERT --split test --max_seq_len 1024 > test_tr_tokenizer.txt
# python dataset/test_data_loader.py --local_path dataset/sample/ --tokenizer answerdotai/ModernBERT-base --split test --max_seq_len 1024 > test_eng_tokenizer.txt

# python dataset/test_data_loader.py --local_path dataset/sample/ --tokenizer boun-tabilab/TabiBERT --split train --max_seq_len 1024 > test_tr_tokenizer.txt
# python dataset/test_data_loader.py --local_path dataset/sample/ --tokenizer answerdotai/ModernBERT-base --split train --max_seq_len 1024 > test_eng_tokenizer.txt